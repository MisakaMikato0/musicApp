...
<template>
	<view class="login-content">
		...
		<view class="iphone">
			...
			<uni-icons class='closeX' type="closeempty" color="#808080" size="25" v-if="showClearIcon" @click="clearIcon"></uni-icons>
		</view>
		...
		<view class="password" v-if="type==2">
			...
			<uni-icons class='eyeX' type="eye-filled" color="#808080" size="25" @click="changePassword"></uni-icons>
		</view>
		...
		methods: {
			...
			Login() {
				...
				if(that.type == 2) {
					...
					data = {
						phone: that.iphoneValue,
						captcha: that.testValue,
					}
				}
				uni.request({
					url: `${that.$baseUrl}/login/cellphone`, // 路径
					method: 'POST', // 请求方法
					data: data, //发送的数据
					success: (res) => {
			...
			getTest() {
				...
				uni.request({
					url: `${that.$baseUrl}/captcha/sent`, // 路径
					method: 'GET', // 请求方法
					data: {
						phone: that.iphoneValue,
					}, //发送的数据
